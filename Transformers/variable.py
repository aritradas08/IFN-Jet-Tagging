...
def build_padded_arrays(merged, label_col):
...
class AttentionMaskLayer(layers.Layer):
    """Computes (B, T, T) boolean attention mask from (B, T) float mask."""
    def call(self, mask):
        # mask: (B, T) float, 1=real 0=pad
        attn = ops.einsum("bi,bj->bij", mask, mask)   # (B, T, T)
        return ops.cast(attn, "bool")

class MaskedMeanPool(layers.Layer):
    """Mean-pools over T dimension, ignoring padded positions."""
    def call(self, x, mask):
        # x: (B, T, D),  mask: (B, T) float
        mask_exp = ops.expand_dims(mask, -1)                      # (B, T, 1)
        summed = ops.sum(x * mask_exp, axis=1)                  # (B, D)
        counts = ops.sum(mask_exp, axis=1)                      # (B, 1)
...
def build_transformer(max_len, feature_dim=5, d_model=32, num_heads=4,
    ...
    mask_in = layers.Input(shape=(max_len,), sname="mask")
...
def run_experiment(label_col, merged_df, epochs=10, batch_size=64):
    ...
    fig.suptitle(f"Training: {label_col}")
    for ax, m, vm in zip(
        axes,
        ["loss", "AUC", "accuracy"],
...
plt.plot([0, 1], [0, 1], "--", color="gray")
plt.xlabel("FPR"); plt.ylabel("TPR")
plt.title("ROC Comparison: IFN vs ATLAS")
